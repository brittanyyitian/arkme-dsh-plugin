...
import type {
  ...
  JotmoRecordingTimelineEvent,
  JotmoRecordingTranscriptItem,
...
export const JOTMO_RECORDING_TOOL_PROMPT =
  ...
  + 'jotmo_recording_read to read exactly one content layer for one date. Always prefer summary or timeline; read transcript only '
  ...
  + 'In user-facing replies, do not expose tool names, cursors, or version ids, internal status codes, or implementation details.'
...
function inclusiveRange(
  from: JotmoRecordingLocalDate,
  to: JotmoRecordingLocalDate,
): { toExclusive: number; count: number } {
  if (from.dateStamp > to.dateStamp) {
    throw new Error('日期范围无效：from_date 不能晚于 to_date')
  }
...
function selectCompletedVersion(
  ...
  requested?: string,
): JotmoRecordingVersion | undefined {
...
function timelineModelItem(item: JotmoRecordingTimelineEvent) {
  return {
...
function renderRecordingData(_args: unknown, value: unknown) {
  return [{
    type: 'text' as const,
    text: `<data_from_jotmo_recording>\n${JSON.stringify(value, undefined, 2)}\n</data_from_jotmo_recording>`,
  }]
}

async function readTranscriptPage(
  service: JotmoRecordingReadService,
  localDate: JotmoRecordingLocalDate,
  continued: JotmoRecordingCursorPayload | undefined,
  limit: number,
  signal: AbortSignal,
) {
  const section = await service.recordingTranscript(localDate.dateStamp, signal)
  ...
  const nextCursor = hasMore
    ? await service.sealRecordingCursor({
      version: 1,
...
function projectionStatePage(
  ...
  sectionState: JotmoRecordingVersionSection['state'],
...
async function readSummaryPage(
  service: JotmoRecordingReadService,
  localDate: JotmoRecordingLocalDate,
  section: JotmoRecordingVersionSection,
  selected: JotmoRecordingVersion,
  availableVersions: ReturnType<typeof safeVersions>,
  continued: JotmoRecordingCursorPayload | undefined,
) {
...
async function readProjectionPage(
  ...
  if (args.content === 'transcript') {
    throw new Error('内部错误：转写内容不能进入投影读取路径')
  }
  const section = await service.recordingProjection(localDate.dateStamp, args.content, signal)
  const availableVersions = safeVersions(section.items)
  let selected: JotmoRecordingVersion | undefined
  if (continued !== undefined) {
    if (continued.versionId === undefined) {
      throw new Error('录音分页游标缺少版本信息')
    }
  ...
  return args.content === 'summary'
    ? await readSummaryPage(service, localDate, section, selected, availableVersions, continued)
...
async function readRecordingPage(
  ...
  const continued = args.cursor === undefined
    ? undefined
    : await service.openRecordingCursor(args.cursor)
  if (continued !== undefined) {
    assertCursorScope(continued, localDate.dateStamp, args.content)
  }
  if (args.content === 'transcript') {
    return await readTranscriptPage(service, localDate, continued, limit ?? 50, signal)
  }
  return await readProjectionPage(service, args, localDate, continued, limit, signal)
}

export function createJotmoRecordingToolDefinitions(
  service: JotmoRecordingReadService,
): ToolDefinition[] {
  return [
    defineTool({
