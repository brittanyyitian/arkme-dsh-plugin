...
export function projectRecordingTranscripts(
  ...
  const projected: Array<JotmoRecordingTranscriptItem & { sourceIndex: number }> = []
  let sourceIndex = 0
...
function normalizeStructuredTimeline(value: unknown): JotmoRecordingTimelineEvent[] {
...
function markdownEvent(event: MutableMarkdownEvent, index: number): JotmoRecordingTimelineEvent {
...
function parseMarkdownTimeline(markdown: string): JotmoRecordingTimelineEvent[] {
...
export function parseRecordingTimeline(value: unknown): JotmoRecordingTimelineEvent[] {
...
export function projectRecordingVersions(
  ...
  const versions: Array<JotmoRecordingVersion & { sourceIndex: number }> = []
  let sourceIndex = 0
  for (const rawVersion of listValue(root.audio_summary_ls ?? root.items ?? root.versions)) {
    ...
    let status: JotmoRecordingVersionStatus = rawStatus === 1 ? 'processing' : rawStatus === 2 ? 'done' : 'failed'
